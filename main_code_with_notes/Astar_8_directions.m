...
while 1
    ...
    for i = 1:8 % 八个邻点
        ...
        if ~isempty(find(ismember(close(:, 1:2), point_n, "rows"), 1))
            j = j + 1; continue; % 在CloseList中有该邻点，跳过该点
        else
            % 在CloseList中无该邻点
...
if ~isempty(open)
    % 找到路径后的操作
    disp("成功通过八向移动的A*算法找到路径！");
    distance = open(1, 6);
    disp(['距离估值为 ', num2str(open(1, 6)),' 。']);
    % 设置回溯初始点
    point_p = open(1, 4:5);
...
draw_path; % 绘制规划路径图
% 保存路径信息
save maze_41x41_astar8_path 'close' 'path' 'path_n' 'timespend' 'distance';