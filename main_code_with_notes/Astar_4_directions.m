...
while 1
    ...
    for i = 1:4 % 八个邻点
        ...
        % 距离终点的代价h 参照函数h_distance，这里采用曼哈顿距离进行衡量
        h = h_distance(point_n, r_g_cord, h_cost, 0);
    ...
    open(1, :) = []; % 将其移除OpenList
    % 对f值进行排序，最小的点在下次优先搜索
%     open(1:(4-j), :) = sortrows(open(1:(4-j), :), 3);
...
draw_path; % 绘制规划路径图
% 保存路径信息
save szdx_yh_astar4_path 'close' 'path' 'path_n' 'timespend' 'distance';